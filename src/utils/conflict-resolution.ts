 ...
 */
async function resolveRootFileConflicts(
 ...
 * Handles root files separately from normal files
 */
export async function resolveFileConflicts(
  ...
  // Resolve root files separately
  const resolvedRootFiles = await resolveRootFileConflicts(rootFiles, targetVersion, silent);

  // Resolve normal files using existing logic
  const resolvedNormalFiles = await resolveNormalFiles(normalFiles, targetVersion, silent);

  // Combine results
  return [...resolvedRootFiles, ...resolvedNormalFiles];
}

/**
 * Resolve normal (non-root) file conflicts
 */
async function resolveNormalFiles(
  discoveredFiles: DiscoveredFile[],
  targetVersion?: string,
  silent?: boolean
): Promise<DiscoveredFile[]> {
...
function createPlatformSpecificPath(file: DiscoveredFile, platformName: string): string | null {
  // Root files always use their original native names, never suffixes
  if (file.isRootFile) {
    return createRootFilePath(platformName);
  }
